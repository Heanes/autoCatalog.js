...
;(function($, window, document, undefined) {
    ...
    AutoCatalog.prototype.bindEvent = function () {
        ...
        let outlineLength = $outlines.length;
        ...
        $(window).on('scroll', function () {
            ...
            for(let i = outlineLength - 1; i > 0; i--){
                let $item = $outlines.eq(i);
                matchScroll = scrollTop >= $item.offset().top - catalogItemHeight;
                if(matchScroll){
                    let chapterCurrent = $item.attr(options.dataChapterFieldName);
                    let $chapterMatched = $catalogList.find('[' + options.dataChapterFieldName + '="' + chapterCurrent + '"]');
                    ...
                    if(overHeight > 0){
                        if(chapterMatchedPosition.top + catalogItemHeight <= catalogListOutHeight){
                            //console.log('top');
                            $catalogList.stop().animate({'top': '0'}, 600);
                            //$catalogListWrap.scrollTop(0);
                            // 右侧操作按钮显示、隐藏
                            $goUpBtn.addClass('disabled');
                            $goDownBtn.removeClass('disabled');
                        }
                        // 如果超出显示范围
                        else if ((chapterMatchedPosition.top + catalogItemHeight > catalogListOutHeight) &&
                            (chapterMatchedPosition.top < overHeight)){
                            //console.log('middle');
                            $catalogList.stop().animate({'top': -chapterMatchedPosition.top + catalogListOutHeight / 2 - catalogItemHeight}, 600);
                            //$catalogListWrap.scrollTop(-(-chapterMatchedPosition.top + catalogListOutHeight / 2 - catalogItemHeight));
                            // 右侧操作按钮显示、隐藏
                            $goUpBtn.removeClass('disabled');
                            $goDownBtn.removeClass('disabled');
                        }else if (chapterMatchedPosition.top > overHeight){
                            //console.log('bottom');
                            $catalogList.stop().animate({'top': -overHeight}, 600);
                            //$catalogListWrap.scrollTop(overHeight);
                            $goUpBtn.removeClass('disabled');
                    ...
                    // 3. 左侧定位滑块滑动-done
                    $scrollbarLocationArrow.stop().animate({'top': chapterMatchedPosition.top + arrowHalfPositionTop}, 100);

                    return false;
                }else{
                    $scrollbarLocationArrow.stop().animate({'top': arrowHalfPositionTop}, 100);
        ...
        $goUpBtn.on('click', function () {
            ...
            if(!$this.hasClass('disabled')){
                if (Math.abs(parseInt($catalogList.css('top'))) >= step) {
                    $catalogList.stop().animate({'top': '+=' + step}, 'fast');
                    $goDownBtn.removeClass('disabled');
                }else{
        ...
        $goDownBtn.on('click', function () {
            let $this = $(this);
            if(!$this.hasClass('disabled')){
                if (overHeight - Math.abs(parseInt($catalogList.css('top'))) >= step) {
                    $catalog.find('.catalog-list').stop().animate({'top': '-=' + step}, 'fast');
                    $goUpBtn.removeClass('disabled');
                }else{
    ...
    AutoCatalog.prototype.renderCatalog = function(treeData) {
        ...
        $catalogContainer.append(this.$catalog);
    ...
    AutoCatalog.prototype.buildTree = function (nodes, level, toSwitch) {
        if (!nodes) return;
        var catalogOption = this.options;
        var options = this.options.treeOption.option;
        // 如果开启切换，则顶级当作0级
        level = level === undefined ? (options.overHeightSwitch ? 0 : 1) : level;
        //console.log(level);

        var _this = this;
        var $treeUl = $(_this.treeTemplate.treeListGroup);

        $.each(nodes, function addNodes(index, node) {
            // console.log('%c ' + level + ' in loop', 'background:#222;color:#bada55');

            var $treeNodeLi = $(_this.treeTemplate.node).attr('data-nodeId', node.nodeId);
            var $treeNodeWrap = $(_this.treeTemplate.nodeWrap);
...

            {
                ...
                if(catalogOption.showChapterSymbol){
                    $treeNodeWrap.append('<em class="outline-pointer"></em>');
                }
    ...
    AutoCatalog.prototype.convertToStandardTree = function (tree) {
        ...
        var defaultNode = options.treeOption.node;
